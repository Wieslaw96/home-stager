...
export default function UpdatePasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      setError("Nie udało się zmienić hasła. Spróbuj ponownie od początku.");
      setLoading(false);
    } else {
      router.push("/app");
    }
    ...
    <div className="min-h-screen bg-[#E8D9C4] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-[#8B6B44]/35 p-8">
          <h1 className="text-2xl font-bold text-[#1A1410] mb-1">Nowe hasło</h1>
          <p className="text-sm text-[#1A1410]/50 mb-6">Ustaw nowe hasło do swojego konta.</p>

          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="block text-sm font-medium text-[#1A1410]/70 mb-1.5">Nowe hasło</label>
              <input
                type="password"
                value={password}
            ...
            <div>
              <label className="block text-sm font-medium text-[#1A1410]/70 mb-1.5">Powtórz hasło</label>
              <input
                type="password"
                value={confirm}
